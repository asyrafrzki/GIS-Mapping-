...
import jabarData from './JABAR_NITROGEN.json';

// ─── FONT IMPORT ─────────────────────────────────────────────────────────────
// Add to your index.html: <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@300;400;500;600;700&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">
...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    if (selectedLevel === null) return jabarData;
    return {
      ...jabarData,
      features: jabarData.features.filter(
        (f) => Number(f.properties.gridcode) === selectedLevel
      ),
    };
  }, [selectedLevel]);
  ...
  const styleGeoJson = (feature) => {
    ...
    return {
      ...
      opacity: 1,
      color: isFiltered ? (isMatch ? '#e31818' : '#333') : 'rgba(255,255,255,0.4)',
      dashArray: '',
  ...
  const onEachFeature = (feature, layer) => {
    ...
    layer.bindPopup(`
      <div style="font-family:'Space Grotesk',sans-serif;min-width:190px;padding:4px;">
...
const stats = useMemo(() => {
  const counts = {};
  jabarData.features.forEach((f) => {
    const code = Number(f.properties.gridcode); // ✅ selalu Number
    if (!code) return; // skip kalau 0/NaN
    counts[code] = (counts[code] || 0) + 1;
  });
  const total = Object.values(counts).reduce((a, b) => a + b, 0);
  
  // Debug — hapus setelah confirmed ok
  console.log('gridcode sample:', jabarData.features[0]?.properties);
    ...
    <div style={styles.root}>
      ...
      <aside style={{ ...styles.sidebar, transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)' }}>
        ...
        <div style={styles.section}>
          ...
          <div style={styles.nutrientGrid}>
            {Object.values(nutrientConfigs).map((n) => (
              <button
                key={n.id}
                onClick={() => { setActiveNutrient(n.id); setSelectedLevel(null); }}
                style={{
                  ...styles.nutrientBtn,
              ...
              >
                <span style={{
                  ...
                  background: activeNutrient === n.id
                    ? n.gradient[2]
        ...
        <div style={styles.sidebarFooter}>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', lineHeight: 1.6 }}>
            Data: Peta Kesuburan Tanah Jabar<br />
            © SoilMap GIS Dashboard
          </div>
        </div>
      </aside>
      ...
      <main style={styles.mapContainer}>
        ...
        >
          ...
          <GeoJSON
            key={`${activeNutrient}-${selectedLevel}`}
            ref={geoJsonRef}
            data={filteredData}
            style={styleGeoJson}
        ...
        <div style={styles.zoomControls}>
          <button className="zoom-btn" style={styles.zoomBtn} onClick={() => document.querySelector('.leaflet-control-zoom-in')?.click()}>+</button>
          <button className="zoom-btn" style={{ ...styles.zoomBtn, borderTop: '1px solid rgba(255,255,255,0.1)' }} onClick={() => document.querySelector('.leaflet-control-zoom-out')?.click()}>−</button>
        </div>
      </main>
    </div>
  );
